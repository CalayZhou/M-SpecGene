...
class LoadRGBTImageFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        try:
            ...
            # if np.max(img)>255 or np.min(img)<0 or np.max(img)-np.min(img)<100:
            #     print(np.max(img), np.min(img))
            #     print(filename)
        ...
        #load img2
        #0410
        filename2 = filename.replace('/imagenet/', '/imagenet2/')
        if '/ir/' in filename2 or '/infrared/' in filename2:
            filename2 = filename2.replace('/ir/', '/rgb/')
            filename2 = filename2.replace('/infrared/', '/visible/')
            #filename2 = filename2.replace('/imagenet/', '/imagenet2/')
        
        '''
        #0410 ori
        if '/ir/' in filename:
            filename2 = filename.replace('/ir/', '/rgb/')
            filename2 = filename2.replace('/infrared/', '/visible/')
        else:
            filename2 = filename.replace('/rgb/', '/ir/')
            filename2 = filename2.replace('/visible/', '/infrared/')
        '''
        ...
        # if random.randint(0, 1) == 0: #0111 calay
