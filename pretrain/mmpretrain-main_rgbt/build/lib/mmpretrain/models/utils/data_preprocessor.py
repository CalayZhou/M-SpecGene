...
class SelfSupDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(
        ...
        data1 = copy.deepcopy(data)  # calay 0108
        data1 = [val for _, val in data1.items()]  # calay 0108
        data = [val for _, val in data.items()]
        # img_t = data1[0][:, 3, :, :]
        # img_t = img_t.unsqueeze(1)#img_t[np.newaxis, :, :]
        # img_t = torch.cat([img_t, img_t, img_t], dim=1)#np.concatenate((img_t, img_t, img_t), axis=0)
        # data1[0] = img_t
        img_t = data1[0][:, 3:6, :, :] #0111 calay
        data1[0] = img_t

        batch_inputs, batch_data_samples = self.cast_data(data)
        batch_inputs1, batch_data_samples1 = self.cast_data(data1)# calay 0108
        ...
        if isinstance(batch_inputs, list):
            ...
            if self._channel_conversion:
                ...
                batch_inputs1 = [
                    _input[:, [2, 1, 0], ...] for _input in batch_inputs1# calay 0108
                ]
            # convert to float after channel conversion to ensure efficiency
            batch_inputs = [_input.float() for _input in batch_inputs]
            batch_inputs1 = [_input.float() for _input in batch_inputs1]# calay 0108

            # normalization.
            if self._enable_normalize:
                batch_inputs = [(_input - self.mean) / self.std
                                for _input in batch_inputs]
                batch_inputs1 = [(_input - self.mean) / self.std
                                for _input in batch_inputs1]# calay 0108
        else:
            # channel transform
            if self._channel_conversion:
                batch_inputs = batch_inputs[:, [2, 1, 0], ...]
                batch_inputs1 = batch_inputs1[:, [2, 1, 0], ...]# calay 0108

            # convert to float after channel conversion to ensure efficiency
            batch_inputs = batch_inputs.float()
            batch_inputs1 = batch_inputs1.float()# calay 0108

            # normalization.
            if self._enable_normalize:
                batch_inputs = (batch_inputs - self.mean) / self.std
                batch_inputs1 = (batch_inputs1 - self.mean) / self.std# calay 0108
        batch_inputs = torch.cat([batch_inputs, batch_inputs1], dim=1)# calay 0108
