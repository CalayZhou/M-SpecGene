...
class MultiheadAttention(BaseModule):
    ...
    def forward(self, x):
        ...
        q, k, v = qkv[0], qkv[1], qkv[2]#32 6 50 64 B head_num N head_dim

        attn_drop = self.attn_drop if self.training else 0.
        x = self.scaled_dot_product_attention(q, k, v, dropout_p=attn_drop)#32 6 50 64
        x = x.transpose(1, 2).reshape(B, N, self.embed_dims)#32  50 384

        x = self.proj(x) #32  50 384
