...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        ###########CALAY 0105 DUAL RGB+T########################
        data1 = copy.deepcopy(data)#data.copy()
        img_t = data1['inputs'][0][3, :, :]
        img_t = img_t.unsqueeze(0)#img_t[np.newaxis, :, :]
        img_t = torch.cat([img_t, img_t, img_t], dim=0)#np.concatenate((img_t, img_t, img_t), axis=0)
        data1['inputs'][0] = img_t
        ...
        if training and self.batch_augments is not None:
            for batch_aug in self.batch_augments:
                ...
                inputs1, data_samples = batch_aug(inputs1, data_samples)

        inputs = torch.cat([inputs, inputs1], dim=1)#calay 0108
