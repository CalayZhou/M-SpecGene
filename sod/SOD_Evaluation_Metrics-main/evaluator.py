...
class Eval_thread():
    ...
    def run(self):
        ...
        print(f"max_f:{max_f}, mean_f:{mean_f},adp_f:{adp_f}")
        mae = self.Eval_MAE()
        print(f'mae:{mae}')
        s_alpha05 = self.Eval_Smeasure(alpha=0.5)
        print(f's:{s_alpha05}')
        max_e, mean_e, adp_e = self.Eval_Emeasure()
        print(f'max_e:{max_e}, mean_e:{mean_e}, adp_e:{adp_e}')
        ...
        # return '[cost:{:.4f}s][{:6} Dataset] [{:6} Method] {:.4f} mae, {:.4f} max-fmeasure, {:.4f} mean-fmeasure, {:.4f} adp-fmeasure\n'\
                # .format(time.time()-start_time, self.dataset, self.method, mae, max_f,mean_f,adp_f)
